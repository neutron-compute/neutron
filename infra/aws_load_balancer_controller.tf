...
resource "helm_release" "aws-load-balancer-controller" {
  ...
  values = [
    templatefile("${path.module}/helm_values/aws_lbc.yaml",
...
resource "kubernetes_service_account_v1" "aws_lbc" {
  metadata {
    name        = "aws-load-balancer-controller"
    namespace   = "kube-system"
    annotations = { "eks.amazonaws.com/role-arn" : aws_iam_role.alb_controller_service_role.arn }
  }
...
data "aws_iam_policy_document" "alb_controller_service_role" {
  statement {
    effect  = "Allow"
...
resource "aws_iam_role" "alb_controller_service_role" {
  name                = "${module.eks.cluster_name}-lb_role"
  assume_role_policy  = data.aws_iam_policy_document.alb_controller_service_role.json
  managed_policy_arns = [aws_iam_policy.load_balancer_policy.arn]
  tags                = var.tags
}
